        ...
        if (cards.selected.stack.split("-")[0] == "t") {
          ...
          if (F.mouse.x < w * data.columns) {
            ...
            ctx.fillRoundRect(
              ...
              (((canvas.height / 2) / data.card_amount) * (cards.table[x].down.length + ((x == cards.selected.stack.split("-")[1]) ? 0 : cards.table[x].up.length) + 1)) - 10,
              w * 0.8,
              ((w * 0.8) * data.card_ratio) + ((cards.table[c].up.length - 1) * ((canvas.height / 2) / data.card_amount) + 10),
              4,
            );
          } else {
            drawCard(
              (data.columns + 0.2) * w,
              ((F.mouse.y - cards.selected.ry) / (((cards.aces.length * 100) / 4))).round().setBorder(0, cards.aces.length - 1) * ((cards.aces.length * 100) / 4) + 10,
              null,
              3,
            );
          }
        ...
        if (cards.selected.stack.split("-")[0] == "t") {
          if (!F.buttonDown(0)) {
            if (cards.drop) {
              ...
              if (F.mouse.x < (canvas.width / (data.columns + 1) - 2) * data.columns) {
                ...
                x1 = parseInt(cards.selected.stack.split("-")[1]);
                c1 = cards.table[x1].up[0].split("-");
                x2 = ((F.mouse.y - cards.selected.ry) / (((cards.aces.length * 100) / 4))).round().setBorder(0, cards.aces.length - 1);
                ...
                if (
                  ...
                  cards.aces[x2] = F.joinArray(cards.aces[x2], cards.table[x1].up);
                  cards.table[x1].up = [];
                }
              }
            }
          ...
          if (F.buttonDown(0)) {
            if (cards.drop) {
              ...
              console.log(cards.table[c].down);
            }
          } else {
            cards.drop = true;
          }
